...
static void init_lock(pthread_mutex_t* lock) {
	err_handler("Cannot initialize mutex: ", pthread_mutex_init(lock, NULL));
}

static void destroy_lock(pthread_mutex_t* lock) {
	err_handler("Cannot destroy mutex: ", pthread_mutex_destroy(lock));
}
